...
### 问题定义
物品分为 `m` 组，每组有若干个物品，每组最多选1个物品，背包容量 `C`，求最大价值。

### 1. 二维DP推导
#### 状态定义
`dp[i][j]`：考虑前 `i` 组物品，背包容量 `j` 时的最大价值。

#### 状态转移方程
对第 `i` 组，有两种选择：「不选任何物品」或「选组内某一个物品」：
- 不选：`dp[i][j] = dp[i-1][j]`
- 选组内第 `k` 个物品（重量 `w[i][k]`，价值 `val[i][k]`）：`dp[i][j] = max(dp[i][j], dp[i-1][j - w[i][k]] + val[i][k])`

### 2. 一维DP优化（逆序遍历容量）
#### 优化思路
压缩为一维数组 `dp[j]`，遍历顺序为：「组→逆序容量→组内物品」。逆序容量的原因同01背包（避免同一组内多个物品被同时选中）。
