...
## 一、核心概念区分（避免混淆）
在开始前，先明确两个关键概念的差异：
| 特征         | 最长回文子串（Longest Palindromic Substring） | 最长回文子序列（Longest Palindromic Subsequence） |
|--------------|-----------------------------------------------|--------------------------------------------------|
| 连续性       | 子串必须连续（字符在原串中紧密相邻）           | 子序列无需连续（字符顺序一致即可，可间隔）         |
| 示例（输入"babad"） | 输出"bab"或"aba"（长度3）                     | 输出"bab"或"aba"（长度3，与子串相同；输入"cbbda"时，子序列为"bcb"或"bdb"，长度3，子串为"bb"长度2） |
| 核心解法     | 中心扩展法（最优，O(n²)时间+O(1)空间） | 动态规划（唯一主流解法，O(n²)时间+O(n²)空间，可优化至O(n)空间） |
