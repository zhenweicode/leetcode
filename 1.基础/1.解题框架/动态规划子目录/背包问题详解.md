...
\[ \text{sum(正)} - \text{sum(负)} = \text{target} \]
\[ \text{sum(正)} + \text{sum(负)} = \text{sum(nums)} \]
两式相加得：\[ 2 \times \text{sum(正)} = \text{sum(nums)} + \text{target} \]
...
**状态转移方程**
对于每个元素 `num`（物品），逆序遍历容量 `j`（01背包，元素不可重复选）：
\[ \text{dp}[j] = \text{dp}[j] + \text{dp}[j - \text{num}] \]
- 含义：`dp[j]` 原有方法数（不选当前元素 `num`） + 选当前元素 `num` 后，凑出 `j - num` 的方法数（`dp[j - num]`）。

**初始化**
`dp[0] = 1`（凑出金额0的方法数为1：不选任何元素），其余 `dp[j] = 0`。
