...
**核心推导：为什么是 `dp[i][j] = dp[i][j - coins[i-1]] + 1`？**
直观理解：「选1枚当前硬币 + 剩余金额的最优解」
对第 `i` 种硬币（面值 `c = coins[i-1]`），要凑成金额 `j` 且「选了至少1枚该硬币」：
- 第一步：先选1枚该硬币，消耗金额 `c`，硬币数 +1；
- 第二步：剩余金额 `j - c` 仍需用「前 `i` 种硬币」凑（因为完全背包可重复选，剩余金额还能再用该硬币）；
- 剩余金额 `j - c` 的最少硬币数是 `dp[i][j - c]`（这是前 `i` 种硬币凑 `j - c` 的最优解）；
- 因此，「选至少1枚第i种硬币」凑成 `j` 的总硬币数 = `dp[i][j - c] + 1`。

而 `dp[i][j]` 是「不选第i种硬币」和「选至少1枚第i种硬币」的最小值，即：
`dp[i][j] = min( dp[i-1][j], dp[i][j - c] + 1 )`（`dp[i-1][j]` 是不选当前硬币的最优解）。
...
给定数组 `nums`，判断能否将其分割为**两个和相等的子集**。
- 数学推导：数组总和 `sum` 必须是偶数（否则无法分割为两个和相等的子集），目标是找到一个子集，其和为 `sum/2`。
- 转化为01背包：每个元素是「物品」，物品的重量和价值都是 `nums[i]`，背包容量是 `target = sum/2`。问题转化为**是否存在一种选法，让背包恰好装满（即子集和等于target）**。

解法1：通过「最值推导」判断存在性（你提供的示例代码）
这种方法将问题转化为**01背包的「最大价值」问题**，再通过「最大价值是否等于target」来判断是否存在解。

动态规划逻辑
- **状态定义**：`dp[j]` 表示容量为 `j` 时，能得到的**最大子集和**。
- **状态转移**：对每个元素 `num`，逆序遍历容量 `j`（避免重复选元素），转移方程为：
  `dp[j] = max(dp[j], dp[j - num] + num)`
  含义：对于容量 `j`，要么「不选当前元素」（保留 `dp[j]` 旧值），要么「选当前元素」（`dp[j - num] + num`，即剩余容量 `j - num` 的最大和 + 当前元素值）。
- **结果判断**：若 `dp[target] == target`，说明存在子集和为 `target`，返回 `true`；否则返回 `false`。
...
解法2：直接用「存在性推导」（true/false 解法）
这种方法更直接，将 `dp` 定义为**布尔数组**，表示「是否能凑出容量 `j`」。

#### 动态规划逻辑
- **状态定义**：`dp[j]` 表示「是否能凑出容量 `j`」（`true` 表示能，`false` 表示不能）。
- **状态转移**：对每个元素 `num`，逆序遍历容量 `j`，转移方程为：
  `dp[j] = dp[j] || dp[j - num]`
  含义：对于容量 `j`，要么「不选当前元素」（`dp[j]` 旧值），要么「选当前元素」（若 `j >= num`，则看 `dp[j - num]` 是否为 `true`）。
- **初始化**：`dp[0] = true`（容量0时，不选任何元素即可凑出，所以为 `true`），其余 `dp[j] = false`。
...
public static boolean canPartition(int[] nums) {
    ...
    if (sum % 2 != 0) return false; // 总和为奇数，直接无解
    int target = sum / 2;
    boolean[] dp = new boolean[target + 1];
    dp[0] = true; // 初始化：容量0可以凑出
    
    for (int num : nums) { // 遍历每个物品（元素）
        for (int j = target; j >= num; j--) { // 逆序遍历容量（01背包）
            dp[j] = dp[j] || dp[j - num];
        }
    }
    return dp[target];
...
一维表示：
- 状态定义：`dp[i]` 表示和为 `i` 时所需的最少完全平方数数量。
- 状态转移方程
对于每个数 `i`，遍历所有小于等于 `i` 的完全平方数 `j²`，则：
\[ dp[i] = \min(dp[i], dp[i - j^2] + 1) \]
- 初始化
`dp[0] = 0`（和为 0 时不需要任何平方数）
其他 `dp[i]` 初始化为 `n + 1`（最大可能值，因为最多需要 `n` 个 1 组成）
...
**解法**：
目标和问题的本质是：在数组 `nums` 中选择一个子集，使得「子集和-剩余元素和 = target」。通过数学推导可得：
\[
sum(正) - sum(负) = targe
\]
\[
...
将两式相加，消去 `sum(负)`：
\[
sum(正) - sum(负) + sum(正) + sum(负) = target + S
\]
...
因此，问题转化为**求子集和等于 `aim = (sum + target) / 2` 的方法数**（需满足 `sum + target` 为偶数且 `sum ≥ |target|`）。

**状态定义**
`dp[j]`：凑出金额 `j` 的**方法数**（这里的「金额」对应 `aim`，即子集和）。

**状态转移方程**
对于每个元素 `num`（01背包，不可重复选），逆序遍历 `j`：
\[
dp[j] = dp[j] + dp[j - num]
\]
- 含义：`dp[j]` 原有值（不选当前元素 `num` 的方法数） + 选当前元素 `num` 后，凑出 `j - num` 的方法数（`dp[j - num]`）。

**初始化**
`dp[0] = 1`（凑出金额0的方法数为1：不选任何元素，关键！！！），其余 `dp[j] = 0`。

```java
public static int findTargetSumWays(int[] nums, int target) {
    int sum = 0;
    for (int num : nums) sum += num;
    // 必须满足sum + target是偶数，且sum >= |target|
    if ((sum + target) % 2 != 0 || Math.abs(target) > sum) {
        return 0;
    }
    int aim = (sum + target) / 2;
    int[] dp = new int[aim + 1];
    dp[0] = 1; // 初始化：凑0的方法数为1（不选任何元素）
    
    for (int num : nums) { // 01背包：逆序遍历容量
        for (int j = aim; j >= num; j--) {
            dp[j] += dp[j - num];
        }
    }
...
这道题是完全背包的「排列数统计」变种，核心是「考虑元素选取顺序，且元素可重复选」。
- **组合数**：不考虑元素顺序，如 `[1,2]` 和 `[2,1]` 视为同一种；
- **排列数**：考虑元素顺序，如 `[1,2]` 和 `[2,1]` 视为两种不同方案。

- 状态定义：`dp[j]`：凑成目标金额 `j` 的**排列数**（所有元素可重复选，且顺序不同算不同方案）。
- 状态转移方程：`dp[j] += dp[j - num]   j >= num`（累加所有能凑成j的前态组合数），含义：凑成 `j` 的排列数 = 所有「选 `num` 作为最后一个元素，再凑成 `j - num`」的排列数之和。
- 初始化：`dp[0] = 1`（凑成金额0的排列数为1：不选任何元素，这是基础情况），其余 `dp[j] = 0`。

**遍历顺序的关键：先容量后物品**
这是**排列数与组合数的核心区别**：
- **组合数（如零钱兑换II）**：先遍历物品，再遍历容量 → 保证“元素顺序固定”，避免重复统计；
- **排列数（本题）**：先遍历容量，再遍历物品 → 允许“同一容量下尝试所有元素作为最后一个元素”，从而体现顺序差异。

**原理示例（`nums = [1,2]`, `target = 3`）：**
- 先遍历容量 `j=1`：
  - 遍历元素 `1`：`dp[1] += dp[0] → dp[1] = 1`（方案 `[1]`）；
  - 遍历元素 `2`：`1 < 2`，无更新；
- 再遍历容量 `j=2`：
  - 遍历元素 `1`：`dp[2] += dp[1] → dp[2] = 1`（方案 `[1,1]`）；
  - 遍历元素 `2`：`dp[2] += dp[0] → dp[2] = 2`（新增方案 `[2]`）；
- 最后遍历容量 `j=3`：
  - 遍历元素 `1`：`dp[3] += dp[2] → dp[3] = 2`（方案 `[1,1,1]`、`[2,1]`）；
  - 遍历元素 `2`：`dp[3] += dp[1] → dp[3] = 2 + 1 = 3`（新增方案 `[1,2]`）；
- 最终 `dp[3] = 3`，对应方案 `[1,1,1]`、`[2,1]`、`[1,2]`，与“排列数”的定义完全一致。
