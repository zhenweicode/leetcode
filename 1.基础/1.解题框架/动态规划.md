...
第二种：
- 计算：$\Sigma dp[i-1][j-k*arr[i-1]](k>=0)= dp[i-1][j]+ \Sigma dp[i-1][j-k*arr[i-1]](k>=1)=dp[i-1][j]+\Sigma dp[i-1][j-(k+1)*arr[i-1]](k>=0)=dp[i-1][j]+\Sigma dp[i-1][j - arr[i-1]-k*arr[i-1]](k>=0)=dp[i-1][j]+dp[i][j - arr[i-1]]$
